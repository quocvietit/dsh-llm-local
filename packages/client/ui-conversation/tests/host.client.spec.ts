...
import {
  CONVERSATION_SETTINGS_NAMESPACE, DEFAULT_BUSY_ENTER_BEHAVIOR, DEFAULT_PLAIN_ENTER_BEHAVIOR, apply,
...
describe('ui-conversation host', () => {
  it('registers, validates, and disposes the durable busy-Enter preference', async () => {
    ...
    expect(ctx.settings.get(ns)).toEqual({
      ...
      plainEnter: DEFAULT_PLAIN_ENTER_BEHAVIOR,
    })
    ...
    await ctx.settings.update(ns, { plainEnter: 'send' })
    expect(ctx.settings.get(ns)).toEqual({ busyEnter: 'steer', plainEnter: 'send' })
    await expect(ctx.settings.update(ns, { busyEnter: 'invalid' })).rejects.toThrow()
    await expect(ctx.settings.update(ns, { plainEnter: 'invalid' })).rejects.toThrow()
    await fiber.dispose()
